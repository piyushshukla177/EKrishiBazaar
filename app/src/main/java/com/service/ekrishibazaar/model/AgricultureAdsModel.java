package com.service.ekrishibazaar.model;

public class AgricultureAdsModel {

    private String user_first_name;
    private String user_last_name;
    private String date_joined;
    private String state;
    private String district;
    private String block;
    private String profile_image;
    private String village_name;
    private String mobile_number;
    private String vid;
    private String product_image1;
    private String product_image2;
    private String product_image3;
    private String price;
    private String posted_on;
    private String post_id;
    private String additional_info;
    private String product_name;
    private String category_name;
    private String category_image;
    private String product_breed;
    private String product_quantity;
    private String product_quantity_by;
    private String product_price_by;
    private String packaging_cost_bearer;
    private String status;

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getUser_first_name() {
        return user_first_name;
    }

    public void setUser_first_name(String user_first_name) {
        this.user_first_name = user_first_name;
    }

    public String getUser_last_name() {
        return user_last_name;
    }

    public void setUser_last_name(String user_last_name) {
        this.user_last_name = user_last_name;
    }

    public String getDate_joined() {
        return date_joined;
    }

    public void setDate_joined(String date_joined) {
        this.date_joined = date_joined;
    }

    public String getState() {
        return state;
    }

    public void setState(String state) {
        this.state = state;
    }

    public String getDistrict() {
        return district;
    }

    public void setDistrict(String district) {
        this.district = district;
    }

    public String getBlock() {
        return block;
    }

    public void setBlock(String block) {
        this.block = block;
    }

    public String getProfile_image() {
        return profile_image;
    }

    public void setProfile_image(String profile_image) {
        this.profile_image = profile_image;
    }

    public String getVillage_name() {
        return village_name;
    }

    public void setVillage_name(String village_name) {
        this.village_name = village_name;
    }

    public String getMobile_number() {
        return mobile_number;
    }

    public void setMobile_number(String mobile_number) {
        this.mobile_number = mobile_number;
    }

    public String getVid() {
        return vid;
    }

    public void setVid(String vid) {
        this.vid = vid;
    }

    public String getProduct_image1() {
        return product_image1;
    }

    public void setProduct_image1(String product_image1) {
        this.product_image1 = product_image1;
    }

    public String getProduct_image2() {
        return product_image2;
    }

    public void setProduct_image2(String product_image2) {
        this.product_image2 = product_image2;
    }

    public String getProduct_image3() {
        return product_image3;
    }

    public void setProduct_image3(String product_image3) {
        this.product_image3 = product_image3;
    }

    public String getPrice() {
        return price;
    }

    public void setPrice(String price) {
        this.price = price;
    }

    public String getPosted_on() {
        return posted_on;
    }

    public void setPosted_on(String posted_on) {
        this.posted_on = posted_on;
    }

    public String getPost_id() {
        return post_id;
    }

    public void setPost_id(String post_id) {
        this.post_id = post_id;
    }

    public String getAdditional_info() {
        return additional_info;
    }

    public void setAdditional_info(String additional_info) {
        this.additional_info = additional_info;
    }

    public String getProduct_name() {
        return product_name;
    }

    public void setProduct_name(String product_name) {
        this.product_name = product_name;
    }

    public String getCategory_name() {
        return category_name;
    }

    public void setCategory_name(String category_name) {
        this.category_name = category_name;
    }

    public String getCategory_image() {
        return category_image;
    }

    public void setCategory_image(String category_image) {
        this.category_image = category_image;
    }

    public String getProduct_breed() {
        return product_breed;
    }

    public void setProduct_breed(String product_breed) {
        this.product_breed = product_breed;
    }

    public String getProduct_quantity() {
        return product_quantity;
    }

    public void setProduct_quantity(String product_quantity) {
        this.product_quantity = product_quantity;
    }

    public String getProduct_quantity_by() {
        return product_quantity_by;
    }

    public void setProduct_quantity_by(String product_quantity_by) {
        this.product_quantity_by = product_quantity_by;
    }

    public String getProduct_price_by() {
        return product_price_by;
    }

    public void setProduct_price_by(String product_price_by) {
        this.product_price_by = product_price_by;
    }

    public String getPackaging_cost_bearer() {
        return packaging_cost_bearer;
    }

    public void setPackaging_cost_bearer(String packaging_cost_bearer) {
        this.packaging_cost_bearer = packaging_cost_bearer;
    }
}
